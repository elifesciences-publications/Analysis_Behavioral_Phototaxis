function [XSource, Xlab, Xfilt, TimeBout, xCoord, yCoord, FishN, DatesUsed, FishUsed, LuminosityBout] ...
    = pooling_loop(Dates, Fish, l, L, exptype, timemin, minBoutNumber)
...
for p = 1: length(Dates) 
    for q = 1: length(Fish(p,:)) 
        ...
        [angleLab, angleSource, angleLabFilt , framerateInfo, coordinates, intensity] = remove_sequence(E, timemin);
        ...
        [intensityBout] = findVarBout(boutIndices, intensity, 2);
        ...
        if ~isempty(seqToDel)
            ...
            intensityBout(seqToDel, :) = [];
        end
        
        %store cum angles
        sizeExp = size(angleBoutSource);
        [row, ~] = find(isnan(XSource),1, 'first');
        
        XSource( row : row + sizeExp(1)-1 , 1 : sizeExp(2)) = deg2rad(angleBoutSource);
        ...
        LuminosityBout( row : row + sizeExp(1)-1 , 1 : sizeExp(2) ) = intensityBout;
...
% delete empty rows
[XSource, delrows] = deleteEmptyRows(XSource);
...
[LuminosityBout] = deleteEmptyRows(LuminosityBout);
