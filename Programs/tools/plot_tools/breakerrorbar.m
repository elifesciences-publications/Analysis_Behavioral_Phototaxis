function h=BreakErrorbar(x,y,err,y_break_start,y_break_end,break_type,y_arbitrary_scaling_factor)
...
ax.YTick = round(ax.YLim(1)): 0.1 : ax.YLim(2);
...
switch break_type
   case {'Patch','RPatch'},
       ...
       patch([xx(:);flipud(xx(:))], ...
           ...
           [.8 .8 .8])
   case 'Line',
       x_half_tick = diff(xtick(1:2))./1.5;
       xx = [xlim(1) xlim(1)+x_half_tick];
       for i=0:2:2
           l = line(xx,y_gap_mid+([-1 2]+i).*y_gap_width./2);
           l.Color = [0 0 0];
           l.LineWidth = 1.5;
       end;
end;
...
% MAP TICKS BACK
i_wrong_ticks = round(ytick,2) > round(y_break_start,2);
ytick = ytick + i_wrong_ticks.*y_break_width;
ytick = unique(sort(round([ytick, y_break_start, y_break_end], 2)));
i_wrong_ticks = round(ytick,2) > round(y_break_start,2);
integer_ticks = all(floor(ytick) == ytick);
label_width = size(yticklabel,2);
if integer_ticks
   format_string = sprintf('%%%d\n',label_width);
else
   left_side = ceil(log10(max(ytick)));
   right_side = label_width-left_side-1;
   format_string = sprintf('%%%dd.%df\n',label_width,right_side);
end;
fictive_ticks = ytick(i_wrong_ticks)-y_break_width;
fictive_ticks(1) = fictive_ticks(1)+y_gap_width(1);
set(gca, 'YTick', [ytick(~i_wrong_ticks) fictive_ticks])
set(gca, 'yticklabel', num2str(round(ytick',2)));