...
Vart1 = wrapTo2Pi(X(:,1:end-1))-pi;
Vart2 = dX(:, :); % dX corrected from individual bias
...
Vart2 = dX(:, 1:end); % dX corrected from individual bias
...
hold on
errorbar(bins+bw/2,  nanmean(meanfish, 2), nanstd(meanfish,1,2)/sqrt(length(unique(FishID))), 'k', 'Linewidth', 1.5)
