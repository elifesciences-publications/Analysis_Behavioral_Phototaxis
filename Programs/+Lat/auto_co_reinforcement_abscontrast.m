function[] = auto_co_reinforcement_abscontrast(dXl, dXi, DIlr, bins, forceonzero)
...
Prodn_np1_spont = dXi(:,1:end-1).*dXi(:,2:end);
Absn_np1_spont = abs(dXi(:,1:end-1)).*abs(dXi(:,2:end));
p_flip_spont = 0.5*(1 - nanmean(Prodn_np1_spont(:))./(0.72*nanmean(Absn_np1_spont(:))));
...
[binvalsA, elts_per_binA, prodbinReinf] = BinsWithEqualNbofElements(abs(Cnp1(reinf)), Prodn_np1(reinf), round(bins/2), round(bins/2)+1);
[binvalsB, elts_per_binB, prodbinInhib] = BinsWithEqualNbofElements(abs(Cnp1(inhib)), Prodn_np1(inhib), round(bins/2), round(bins/2)+1);
[binvalsC, elts_per_binC, ACbinReinf] = BinsWithEqualNbofElements(abs(Cnp1(reinf)), ACn_np1(reinf), round(bins/2), round(bins/2));
[binvalsD, elts_per_binD, ACbinInhib] = BinsWithEqualNbofElements(abs(Cnp1(inhib)), ACn_np1(inhib), round(bins/2), round(bins/2));
...
%% Pflip
[binvalsA, elts_per_binA, absbinReinf] = BinsWithEqualNbofElements(abs(Cnp1(reinf)), Absn_np1(reinf), round(bins/2), round(bins/2)+1);
[binvalsB, elts_per_binB, absBinInhib] = BinsWithEqualNbofElements(abs(Cnp1(inhib)), Absn_np1(inhib), round(bins/2), round(bins/2)+1);
...
%***
figure
plot([binvalsA], ones(length(binvalsA),1)*p_flip_spont, '--',...
    'Linewidth', 1.5, 'DisplayName', 'spontaneous value', 'Color', colour(2,:))
hold on
